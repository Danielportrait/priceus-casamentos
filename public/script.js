document.addEventListener("DOMContentLoaded", function () {
  ...
  window.enviarWhatsApp = function enviarWhatsApp() {
    ...
    const nomeNoivo = document.getElementById("nomeNoivo")?.value || "Não informado";
    ...
    const dataCasamento = document.getElementById("dataCasamento")?.value || "Não informado";
    const cidade = document.getElementById("cidadeCasamento")?.value || document.getElementById("outraCidade")?.value || "Não informado";
    const estado = document.getElementById("estadoCasamento")?.value || document.getElementById("outroPais")?.value || "Não informado";
    const totalPacote = document.getElementById("valorTotalFinal")?.textContent || "Não informado";
    const formaPagamento = document.getElementById("formaPagamento")?.value || "Não especificada";

    const servicosSelecionados = Array.from(document.querySelectorAll('#servicosSelecionados input[type="checkbox"]:checked'))
        .map(checkbox => checkbox.nextElementSibling?.textContent.trim() || "Serviço não identificado");
    ...
    if (formaPagamento === "pix") {
        ...
        const parcelasCartao = document.getElementById("parcelasCartao")?.textContent || "Não especificado";
        pagamentoOpcao = `Pagamento via Cartão de Crédito: ${parcelasCartao}.`;
    } else if (formaPagamento === "avista") {
        const valorAvista = document.getElementById("valorAvista")?.textContent || "0,00";
        pagamentoOpcao = `Pagamento à vista com 10% de desconto: R$ ${valorAvista}.`;
    } else {
        pagamentoOpcao = "Forma de pagamento não especificada.";
    }

    const mensagem = `
👋 Olá, tudo bem? Gostaria de compartilhar o orçamento para o casamento:
...
💳 *Forma de Pagamento*: ${pagamentoOpcao}
